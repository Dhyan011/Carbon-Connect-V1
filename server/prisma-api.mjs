...
function send(req, res, status, data, error = null) { headers(req, res); res.statusCode = status; res.end(JSON.stringify({ data, meta: { requestId: randomUUID(), timestamp: new Date().toISOString() }, error })); }
function fail(req, res, status, code, message) { return send(req, res, status, null, { code, message }); }
async function readBody(req) { let raw = ''; for await (const chunk of req) raw += chunk; return raw ? JSON.parse(raw) : {}; }
async function currentUser(req) { const claims = verifyAccessToken((req.headers.authorization || '').replace(/^Bearer\s+/i, '')); if (!claims) return null; return prisma.user.findUnique({ where: { id: claims.sub }, include: { memberships: { include: { organization: true } } } }); }
...
const server = http.createServer(async (req, res) => {
  ...
  if (!req.url.startsWith('/api/v1/')) return fail(req, res, 404, 'NOT_FOUND', 'API route not found.');
  try {
    if (req.url === '/api/v1/auth/login' && req.method === 'POST') {
      const input = await readBody(req); const user = await prisma.user.findUnique({ where: { email: input.email }, include: { memberships: true } }); const match = user?.memberships.find(m => m.role.startsWith(String(input.role || '').toUpperCase())) || user?.memberships[0];
    ...
    const rfqBids = req.url.match(/^\/api\/v1\/rfqs\/([^/]+)\/bids$/);
    if (rfqBids && req.method === 'GET') { const rfq = await prisma.rfq.findUnique({ where: { rfqCode: rfqBids[1] } }); if (!rfq) return fail(req, res, 404, 'RFQ_NOT_FOUND', 'RFQ not found.'); const rows = await prisma.bid.findMany({ where: { rfqId: rfq.id }, include: { rfq: true, listing: true, sellerOrganization: true }, orderBy: { createdAt: 'asc' } }); return send(req, res, 200, rows.map(mapBid)); }
    if (rfqBids && req.method === 'POST') { const m = membership(user, 'seller'); if (!m) return fail(req, res, 403, 'FORBIDDEN', 'Seller membership required.'); const rfq = await prisma.rfq.findUnique({ where: { rfqCode: rfqBids[1] } }); if (!rfq || !['PUBLISHED', 'BIDS_RECEIVED'].includes(rfq.status)) return fail(req, res, 409, 'RFQ_NOT_OPEN', 'RFQ is not accepting bids.'); const input = await readBody(req); const listing = await prisma.listing.findFirst({ where: { organizationId: m.organizationId, status: 'PUBLISHED' } }); if (!listing) return fail(req, res, 409, 'LISTING_REQUIRED', 'A published seller listing is required.'); const duplicate = await prisma.bid.findFirst({ where: { rfqId: rfq.id, sellerOrganizationId: m.organizationId, status: { in: ['SUBMITTED', 'SHORTLISTED'] } } }); if (duplicate) return fail(req, res, 409, 'DUPLICATE_BID', 'Your organization already has an active bid.'); const exWorks = Number(String(input.price || '0').replace(/[^0-9.]/g, '')); const delivered = Number(String(input.delivered || exWorks + 6).replace(/[^0-9.]/g, '')); const created = await prisma.bid.create({ data: { bidCode: `CC-BID-${Date.now()}`, rfqId: rfq.id, sellerOrganizationId: m.organizationId, listingId: listing.id, quantityTonnes: Number(String(input.quantity || rfq.quantityTonnes).replace(/[^0-9.]/g, '')), exWorksPriceInrPerTonne: exWorks, logisticsPriceInrPerTonne: Math.max(0, delivered - exWorks), deliveredPriceInrPerTonne: delivered, totalEstimatedValueInr: delivered * Number(rfq.quantityTonnes), leadTimeBusinessDays: Number(String(input.leadTime || '5').replace(/[^0-9]/g, '')) || 5, evidenceSummary: input.evidence || 'Evidence under review', status: 'SUBMITTED', submittedAt: new Date() }, include: { rfq: true, listing: true, sellerOrganization: true } }); await prisma.rfq.update({ where: { id: rfq.id }, data: { status: 'BIDS_RECEIVED' } }); return send(req, res, 201, mapBid(created)); }
